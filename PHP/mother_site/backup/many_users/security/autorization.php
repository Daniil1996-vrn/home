...
$code_auth.=1;
$code_auth.=sha1($code_auth);


  
$query ='SELECT login,password,status,groups FROM users WHERE login='."'". $login."' AND password='". $password2."'";
//echo $query;
...
if($login==$row['login'] &&$password2==$row['password'] && $row['status']=='1' && $row['groups']=='u' ) 
{
$prov=1;
$query2 = "UPDATE users SET code_auth=$code_auth WHERE login=" ."'$login' and password="."'$password2'";
 "'$code_auth'";
//echo $query2;
$conn->query($query2);
...
setcookie("code_auth",$code_auth,2147483647);
}
 if($prov==0) echo <<<_END
<!DOCTYPE html>
<html lang="en">
